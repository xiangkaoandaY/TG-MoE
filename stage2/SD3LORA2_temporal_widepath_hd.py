...
def save_ft_probe_pipeline_visuals(
    ...
    for prompt_index in range(len(images) // 2):
        ...
        # 保存Pipeline原生分辨率图。PNG本身是无损格式；显式使用
        # compress_level=0，防止后续为了拼时间线而覆盖或缩小单图。
        image_wm.save(
            wm_path,
            format="PNG",
            compress_level=0,
    ...
    with accelerator.autocast():
        ...
        for i in range(args.num_validation_images):
            ...
            latents_wm = pipeline(
                ...
                height=args.ft_probe_generation_resolution,
                width=args.ft_probe_generation_resolution,
                num_inference_steps=args.ft_probe_num_inference_steps,
                generator=gen_wm,
                guidance_scale=args.ft_probe_guidance_scale,
                output_type="latent",
...
if __name__ == "__main__":
    def parse_args():
        ...
        parser.add_argument(
            "--ft_probe_generation_resolution",
            type=int,
            default=1024,
            help="Native square resolution used for LoRA-probe Watermarked/Clean images.",
        )
        parser.add_argument(
            "--ft_probe_num_inference_steps",
            type=int,
            default=28,
            help="SD3 denoising steps used for every fixed-seed LoRA-probe image.",
        )
        parser.add_argument(
            "--ft_probe_guidance_scale",
            type=float,
            default=7.0,
            help="Fixed CFG scale used for all LoRA-probe image milestones.",
        )
        parser.add_argument(
